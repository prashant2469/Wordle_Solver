...
char *get_unique_array(char *guess){
  char *k = (char*)malloc(strlen(guess) * sizeof(char));
  k = strdup(guess);
...
int score_word(char *word, int *letter_scores) {
  int score = 0;
  int index;
  char *unique_word = get_unique_array(word);
  for(int i = 0; i < 5; i++){
  	if(unique_word[i] == 0){
		  continue;
    }
    score = letter_scores[unique_word[i] - 'a'] + score;
  }
  free(unique_word);
  unique_word = NULL;
  return score;
...
size_t filter_vocabulary_yellow(char letter, int position, char **vocabulary, size_t num_words) {
  ...
  for(size_t i = 0; i < num_words; i++){
    ...
    if(!strchr(vocabulary[i], letter) || vocabulary[i][position] == letter){
      free(vocabulary[i]);
      vocabulary[i] = NULL;
      count++;
    }
